...
При старте контейнер `api` делает `prisma db push` (синхронизация схемы), затем
идемпотентный `prisma/reconcile.ts` (правит устаревшие дефолты на существующих
данных — например, опускает `minBet` рулетки 0.1 → 0.01 — не затирая правки
админа), а при `SEED_ON_START=true` — сид: валюты, игры (рулетка + тайтлы «скоро»),
VIP, **аккаунт админа**, дефолтные права ролей (SUPPORT/MODERATOR), контент-страницы,
промокоды, демо-розыгрыш.

> Разовая правка существующей БД без полного пере-сида: `pnpm --filter @kukumba/api db:reconcile`.
