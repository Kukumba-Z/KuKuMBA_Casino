 ...
 */
  ...
  export class CrashEngine {
    ...
    drawBubble(ctx, W, H, look, t) {
      ...
      // Держим облачко чуть правее и НИЖЕ нижней кромки большого множителя, чтобы
      // реплика («бууэ…», фразы) не перекрывала икс.
      const fs = clamp(W * 0.11, 42, 100);
      const multBottom = Math.max(H * 0.15, 60 + fs * 0.5) + fs * 0.62;
      let bx = W / 2 + 96 * scale + bw / 2 - 30;
      bx = Math.min(bx, W - bw / 2 - 10);
      let by = headTop - bh / 2 - 6 + Math.sin(t * 1.6) * 3;
      by = Math.max(by, bh / 2 + 8, multBottom + bh / 2 + 6);
