...
- **Капа на множитель НЕТ** — лестница честная до самого конца. Естественный
  максимум формулы: полная зачистка при m=12 или m=13 даёт `C(25,12) = 5 200 300`,
  т.е. ×~5.15M при RTP 0.99 (референс-казино при RTP 97% показывают ровно
  ×5.04M, а шаги до него — ×48.04k и ×360.31k; наши значения обязаны совпадать
  с ними при том же RTP). Максимальная экспозиция = `maxBet × 5.2M` —
  ограничивается ставочными лимитами (`maxBet` игры правится в админке,
  плюс `tableMaxStake`), а не обрезанием множителя.
...
- `multiplierFor(m, k, rtp): number` — формула из §2, без капа.
...
- Формула множителя: ручные значения (например m=3,k=1 → RTP×25/22), инкремент
  против прямого биномиального расчёта; якорные значения индустрии при RTP 0.97
  и m=13: k=10 → ≈48 041 (×48.04k), k=11 → ≈360 306 (×360.31k), k=12 →
  ≈5 044 291 (×5.04M); при RTP 0.99 полная зачистка m=12/13 → ≈5 148 297.
...
- **Aside, карточка управления:** ставка (инпут c паттерном Upgrader: очистка
  ...
  и сумма кэшаута. Главная кнопка (`crash-action
...
- **Сцена, карточка:** сетка `grid grid-cols-5` из 25 кнопок-плиток,
  ...
  Над полем — два чипа-счётчика: сколько безопасных клеток осталось (иконка
  монеты/кристалла, mint) и сколько мин на поле (иконка бомбы, roul-red).
- **Лента «Следующий шаг»** под полем (как у больших казино): горизонтально
  скроллируемая полоса чипов с ПРЕДСТОЯЩИМИ множителями лестницы (из
  `multipliers` в info/view), текущая позиция подсвечена, по краям стрелки
  ‹ ›; при открытии клетки лента сдвигается на шаг. Рядом — сумма кэшаута
  следующего шага. Формат множителей компактный: `fmtMult`: до 1000 —
  `×12.55`, тысячи — `×48.04k`, миллионы — `×5.04M` (при 13 минах хвост
  лестницы обязан выглядеть как ×48.04k → ×360.31k → ×5.04M на RTP 97%).
  Той же функцией форматируй чипы «Последних раундов».
...
4. Смена RTP в админке (Настройки → панель RTP) меняет множители **новых**
   ...
   Контроль лестницы: выставь RTP 97% и 13 мин — хвост ленты обязан показать
   ×48.04k → ×360.31k → ×5.04M (сверка с индустрией); на дефолтных 99% полная
   зачистка при 12–13 минах ≈ ×5.15M.
