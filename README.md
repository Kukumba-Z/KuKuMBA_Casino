...
Онлайн-казино на NestJS + React. Сегодня две полностью рабочие игры — **provably-fair
рулетка** и **crash-игра VODKA WIN** — плюс **каталог игр** с карточками будущих тайтлов
от провайдеров (статус «скоро»).
...
- **VODKA WIN Crash** — одиночная crash-игра (canvas-сцена с карикатурным героем, до ×1 000 000):
  множитель растёт по детерминированной кривой, забери выигрыш до краша. Точка краша — provably-fair
  из **той же цепочки сидов**, что у рулетки (`P(crash ≥ m) = RTP / m` ⇒ плоский edge на любой цели),
  фиксируется в транзакции ставки и **не покидает сервер** до конца раунда; кэшаут валидируется
  серверным временем (множителю клиента не доверяем), авто-кэшаут и брошенные раунды дорешивает
  свипер. RTP **настраивается** (по умолчанию **99%**), турбо-режим = общий «Быстрая игра».
...
- **Бонусы**, **промокоды**, **рефералка** (revenue share: 10% от чистых проигрышей рефералов,
  с отрицательным переносом), **VIP** (21 уровень в стиле My Little Pony; прокачка депозитами И
  ставками — ставок нужно в 5 раз больше), **кешбэк** раз в неделю ((депозиты − выводы за 7 дней) ×
  процент уровня, вейджер ×3), **рейкбэк** (доля от house edge с каждой реальной ставки, без отыгрыша).
- **Розыгрыши** — от админа/партнёров; призовой фонд и кол-во победителей, provably-fair выбор;
  условия входа: депозит, VIP-уровень, аудитория партнёра.
...
│  │  └─ src/modules/*             # auth, users, games/{roulette,crash}, wallet, payments, raffles,
...
3. Исход рулетки: `floor( f(HMAC_SHA256(serverSeed, "clientSeed:nonce")) × 37 )`;
   исход crash: `min(1e6, floor2( (1−edge) / (1−f) ))` из того же float `f` (мгновенный краш при
   `f < edge`).
4. При смене сида старый `serverSeed` раскрывается — историю можно перепроверить
   (`POST /api/provably-fair/verify` возвращает и номер ячейки рулетки, и точку краша).
5. RTP задаётся выплатами: в рулетке `multiplier(bet) = RTP / probability(bet)`, в crash
   `P(crash ≥ m) = RTP / m` — матожидание любой ставки равно `ставка × RTP`. Значение
   настраивается: глобально в `game.rtp` и/или у конкретной игры (поле `Game.rtp`), без передеплоя.
...
## Тесты
```bash
pnpm test     # юнит-тесты: payout-математика рулетки и crash (плоский RTP на любой цели,
              # кривая роста и её инверсия) + provably-fair RNG (детерминизм, равномерность)
```
